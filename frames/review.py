...
class Review(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # 1 ------------------------------------------------------------------------------
        self.label_col = ttk.Label(self, text="Train Images", font=MEDIUMFONT)
        self.label_col.grid(row=1, column=0, padx=0, pady=0)
        # self.label_name = ttk.Label(self, text="[image]", font=MEDIUNFONT)
        self.label_inf = ttk.Label(self, text="Current Label", font=MEDIUMFONT)
        self.label_inf.grid(row=1, column=2, padx=0, pady=0)

        # 2 ------------------------------------------------------------------------------
        var = tk.Variable(value=self.train_images)
    ...
    def image_selected(
        ...
        self.label_name['text'] = self.train_images[selected_index]
        image = utils.prepare_image(self.train_files[selected_index], self.image_side)
        ...
        image_class = self.predictor.predict(self.train_files[selected_index])
